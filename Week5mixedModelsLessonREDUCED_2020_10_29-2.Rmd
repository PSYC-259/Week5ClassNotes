...
1. Import the multilevel data from this source "https://web.archive.org/web/20160624172041if_/http://www-personal.umich.edu/~bwest/classroom.csv"
School variables:
...
```{r}
df <- read.csv("~/Desktop/schoolsClassStudents.csv")
```

2. Scale and center your mathgain, ses, and mathprep variables.

```{r}
df$zSES <- scale(df$ses)
df$zMathgain <- scale(df$mathgain)
df$zMathprep <- scale(df$mathprep)
```
...
```{r}
m1 <- lmer(zMathgain ~ 1 + (1 | classid), data = df)
summary(m1)
```
...
```{r}
m2 <- lmer(zMathgain ~ zSES + (1 | classid), data = df)
summary(m2)
```
...
```{r}
m3 <- lmer(zMathgain ~ zSES + (zSES | classid), data = df)
summary(m3)
```
...
```{r}
m4 <- lmer(zMathgain ~ zMathprep + (1 | classid), data = df)
summary(m4)
```
...
```{r}
m5 <- lmer(zMathgain ~ zMathprep * zSES + (1 | classid), data = df)
summary(m5)
```
...
```{r}
m6 <- lmer(zMathgain ~ zMathprep * zSES + (zSES | classid), data = df)
summary(m6)
```
