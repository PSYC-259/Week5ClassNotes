...
```{r}
dat<-read.csv("~/Desktop/Psyc259:StatisticsinR/PSYC259-CommonAnalysisData.csv")
```
...
```{r}
library(tidyr)
datLong<-pivot_longer(dat, cols = c("evaluationExclusion", "evaluationInclusion"),names_to = "playerEvaluated", values_to = "evaluationScore")
```
This changes the t test arguments to: 
t.test(formula, data)
```{r}
t.test(evaluationScore~playerEvaluated, data=datLong)
```
...
```{r}
t.test(evaluationScore~playerEvaluated, data=datLong, paired=TRUE )
```
...
```{r}
t.test(evaluationScore~playerEvaluated, data=datLong, paired=TRUE, var.equal=TRUE)
```

Finally, we some times run one tailed vs two tailed tests, just like we did with the correlations. 
```{r}
t.test(evaluationScore~playerEvaluated, data=datLong, paired=TRUE, var.equal=TRUE, alternative="less")
```
...
lm(DV ~ IV, data)
```{r}
lm1<-lm(evaluationScore~playerEvaluated, data=datLong)
```
...
```{r}
lm2<- lm(evaluationScore~playerEvaluated + age, data=datLong)
summary(lm2)
```
The : can be used instead of + to include an interaction in your model
```{r}
lm3<- lm(evaluationScore~playerEvaluated : age, data=datLong)
summary(lm3)
```
...
```{r}
datLong$age<-as.factor(datLong$age)
lm4<- lm(evaluationScore~playerEvaluated * age, data=datLong)
summary(lm4)
```
...
```{r}
datLong$playerEvaluated<- as.factor(datLong$playerEvaluated)
levels(datLong$playerEvaluated)
datLong$playerEvaluated<-relevel(datLong$playerEvaluated, ref="evaluationInclusion")
lm5<- lm(evaluationScore~playerEvaluated, data=datLong)
summary(lm5)
```
...
```{r}
lm6<- lm(evaluationScore~playerEvaluated+gender, data=datLong)
aov(lm6)
anova(lm6)
library(car)
Anova(lm6)
...
```{r}
dat$characterChoice<- ifelse(dat$playChoice=="Includer",1 ,0)
summary(glm(characterChoice~evaluationInclusion, data=dat,family = "binomial"))
```
...
summary(glm(characterChoice~scale(evaluationInclusion), data=dat,family = "binomial"))
```
...
```{r}
biserial.cor(dat$evaluationExclusion, dat$characterChoice)
```
Mathematically, this is the same as the pearson's version. 
```{r}
cor.test(dat$evaluationExclusion, dat$characterChoice)
```
...
wilcox.test
```{r}
wilcox.test(dat$evaluationExclusion, dat$evaluationInclusion, paired=TRUE)
```
